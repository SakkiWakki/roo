use super::ffi::{
    ...
    PFN_vkCreateFence, PFN_vkCreateFramebuffer, PFN_vkCreateGraphicsPipelines, PFN_vkCreateImage,
    PFN_vkCreateInstance, PFN_vkCreatePipelineLayout, PFN_vkCreateRenderPass,
    ...
    PFN_vkGetPhysicalDeviceProperties2, PFN_vkQueueSubmit, PFN_vkResetFences, PFN_vkWaitForFences,
    VkDevice, VkInstance,
};
...
impl<'a> VulkanLoader<'a> {
    pub fn load(loader: &'a LinuxLoader) -> Self {
        let get_instance_proc_addr =
            unsafe { loader.symbol::<PFN_vkGetInstanceProcAddr>("vkGetInstanceProcAddr") }.unwrap();
        let create_instance =
            unsafe { loader.symbol::<PFN_vkCreateInstance>("vkCreateInstance") }.unwrap();
        Self {
...
impl InstanceLoader {
    pub fn load(vk: &VulkanLoader, instance: VkInstance) -> Self {
        let load = |name: &[u8]| unsafe {
            ...
            proc(instance, name.as_ptr() as _).expect("missing instance function")
        };
        unsafe {
            Self {
                enumerate_physical_devices: std::mem::transmute(load(
                    b"vkEnumeratePhysicalDevices\0",
                )),
                get_physical_device_properties2: std::mem::transmute(load(
                    b"vkGetPhysicalDeviceProperties2\0",
                )),
                get_physical_device_memory_properties: std::mem::transmute(load(
                ...
                create_debug_utils_messenger: std::mem::transmute(load(
                    b"vkCreateDebugUtilsMessengerEXT\0",
                )),
                destroy_debug_utils_messenger: std::mem::transmute(load(
...
impl DeviceLoader {
    pub fn load(instance: &InstanceLoader, device: VkDevice) -> Self {
        let load = |name: &[u8]| unsafe {
            ...
            proc(device, name.as_ptr() as _).expect("missing device function")
        };
        unsafe {
            Self {
                get_device_queue: std::mem::transmute(load(b"vkGetDeviceQueue\0")),
                ...
                get_image_memory_requirements: std::mem::transmute(load(
                    b"vkGetImageMemoryRequirements\0",
                )),
                bind_image_memory: std::mem::transmute(load(b"vkBindImageMemory\0")),
                get_memory_fd: std::mem::transmute(load(b"vkGetMemoryFdKHR\0")),
                create_render_pass: std::mem::transmute(load(b"vkCreateRenderPass\0")),
                create_framebuffer: std::mem::transmute(load(b"vkCreateFramebuffer\0")),
                create_graphics_pipelines: std::mem::transmute(load(
                ...
                cmd_begin_render_pass: std::mem::transmute(load(b"vkCmdBeginRenderPass\0")),
                cmd_end_render_pass: std::mem::transmute(load(b"vkCmdEndRenderPass\0")),
                cmd_draw: std::mem::transmute(load(b"vkCmdDraw\0")),
                queue_submit: std::mem::transmute(load(b"vkQueueSubmit\0")),
                create_fence: std::mem::transmute(load(b"vkCreateFence\0")),
                wait_for_fences: std::mem::transmute(load(b"vkWaitForFences\0")),
                reset_fences: std::mem::transmute(load(b"vkResetFences\0")),
